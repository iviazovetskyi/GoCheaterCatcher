...
class LeelaZeroAnalysis():
    def run_analysis(self, current_move):
        ...
        next_moves = [d['first move'] for d in position_evaluation['variations'] if 'first move' in d]
        ...
        return best_answer, next_moves, position_evaluation
...
class Leela_Zero_gtp(gtp):
    def get_heatmap(self):
        ...
        for i in range(self.size + 2):
            ...
            new_values = [[letter_coordinate + str(number_coordinate), int(value) / 1000.] for letter_coordinate, value
                          in zip(letters, one_line)]
    ...
    def quick_evaluation(self, color):
        ...
        txt = (black_win_rate + '/' + white_win_rate)
        txt += "\n\n" + self.get_leela_zero_final_score()
        return txt
            ...
            try:  # for comptability with Leela Zero dynamic komi
                ...
                if " ->" in err_line:
                    if err_line[0] == " ":
                        ...
                        variation[
                            "value network win rate"] = value_network  # for Leela Zero, the value network is used as win rate
